<template>
  <section style="overflow: auto;">
    <div style="width: 100%" v-if="showTitle">
      ...
      <span style="font-size: small; color: gray;">共查出<span style="color: black">{{total}}</span>名会员</span>
    </div>
    ...
    >
      ...
      <el-table-column
        ...
        :label="col.label"
        :min-width="col.width"
        :show-overflow-tooltip="col.tooltip == '1' ? true : false"
        :formatter="formatter"
        :sortable="col.enableOrder == '1' ? true : false"
      >
        <template v-if="!!col.children && col.children.length > 0">
          <el-table-column
            v-for="c1 in col.children"
            :prop="c1.field"
            :label="c1.label"
            :min-width="c1.width"
            :show-overflow-tooltip="c1.tooltip == '1' ? true : false"
            :formatter="formatter"
            :sortable="c1.enableOrder == '1' ? true : false"
          >
          </el-table-column>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="rowFuncs.length > 0" prop="x" label="操作" :width="calcOperateColWidth">
        <template scope="scope">
          <div v-for="(func, idx) in rowFuncs" style="display: inline;">
            <el-button
              v-if="showRowFunc(scope.row, func)"
              type="text"
              @click="func.action(scope.row)"
    ...
    <div class="selection" v-if="showSelectionTags">
      已选择：
      <el-tag  style="margin-left: 5px; margin-right: 5px; margin-top:2px; margin-bottom:2px;"
        closable
        @close="handleClose(idx)"
        v-for="(obj, idx) in allSels">
        {{ obj[labelKey] }}
      </el-tag>
    </div>
